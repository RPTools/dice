...
class DieRollTest {
  ...
  private static final EnumSet<DieRoll.DieRollFlags> SUCCESS =
      EnumSet.of(DieRoll.DieRollFlags.SUCCESS);
  private static final EnumSet<DieRoll.DieRollFlags> FAILURE =
      EnumSet.of(DieRoll.DieRollFlags.FAILURE);
  private static final EnumSet<DieRoll.DieRollFlags> FUMBLE =
      EnumSet.of(DieRoll.DieRollFlags.FUMBLE);
  private static final EnumSet<DieRoll.DieRollFlags> CRITICAL =
      EnumSet.of(DieRoll.DieRollFlags.CRITICAL);

  private static final EnumSet<DieRoll.DieRollFlags> ALL =
      EnumSet.copyOf(Arrays.asList(DieRoll.DieRollFlags.values()));

  /**
   * Helper function to create a {@link DieRoll} with a successful flag.
   *
   * @param val The result of the dice roll.
   * @return a {@link DieRoll} with a successful flag.
   */
  private static DieRoll successFlagRoll(int val) {
    return new DieRoll(val, SUCCESS, val + 1);
  }
  ...
  private static DieRoll failureFlagRoll(int val) {
    return new DieRoll(val, FAILURE, 2 * val);
  }
  ...
  private static DieRoll fumbleFlagRoll(int val) {
    return new DieRoll(val, FUMBLE, val * 4);
  }
  ...
  private static DieRoll criticalFlagRoll(int val) {
    return new DieRoll(val, CRITICAL, val);
  }
  ...
  private static DieRoll allFlagsRoll(int val) {
    return new DieRoll(val, ALL, val);
  }
    ...
    random
        ...
        .forEach(
            r -> {
              ...
              // All
              assertTrue(
                  drAll.getFlags().containsAll(Arrays.asList(DieRoll.DieRollFlags.values())));
            });
  }
