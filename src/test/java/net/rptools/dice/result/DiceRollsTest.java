...
class DiceRollsTest {
   ...
   *
   * @param number The number of rolls to add.
  ...
  private static Collection<DieRoll> generateRolls(int number, int sides) {
    ...
    return random
        .ints(number, 1, sides + 1)
        .mapToObj(r -> new DieRoll(r, sides))
        .collect(Collectors.toList());
  }

  /**
   * Helper function to generate a {@link DiceRolls}s which have been flagged with different
   * statuses.
   *
   * <p>Rolls generated. 1 - fumble, failure 2 - failure 3 - no flags 4 - no flags 5 - success 6 -
   * critical, success
   *
  ...
  private static DiceRolls generateFlaggedRolls() {
    ...
    dieRolls.add(
        new DieRoll(1, List.of(DieRoll.DieRollFlags.FUMBLE, DieRoll.DieRollFlags.FAILURE), 6));
    dieRolls.add(new DieRoll(2, Collections.singleton(DieRoll.DieRollFlags.FAILURE), 6));
    dieRolls.add(new DieRoll(3, Collections.emptyList(), 6));
    dieRolls.add(new DieRoll(4, Collections.emptyList(), 6));
    dieRolls.add(new DieRoll(5, Collections.singleton(DieRoll.DieRollFlags.SUCCESS), 6));
    dieRolls.add(
        new DieRoll(6, List.of(DieRoll.DieRollFlags.CRITICAL, DieRoll.DieRollFlags.SUCCESS), 6));
  ...
  void getNumberOfSides() {
    ...
    diceRolls =
        new DiceRolls(
            Collections.singleton(new DieRoll(6, 6)),
            10,
  ...
  void getResult() {
    var diceRolls =
        new DiceRolls(
            Collections.singleton(new DieRoll(1, 6)),
            10,
